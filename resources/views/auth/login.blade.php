...
<head>
    ...
    <title>Connexion — VigilCore Platform</title>
...
<body class="bg-[#f8fafc] dark:bg-[#090d16] text-slate-900 dark:text-slate-100 font-sans antialiased min-h-screen flex items-center justify-center p-4 selection:bg-[#0020B2] selection:text-white transition-colors duration-200">
    ...
    <!-- ==================================================== -->
    <!-- CARTE D'AUTHENTIFICATION ÉPURÉE (STYLE ZABBIX / DATADOG) -->
    <!-- ==================================================== -->
    <div class="w-full max-w-[420px] bg-white dark:bg-[#111827] rounded-2xl border border-slate-200 dark:border-slate-800 shadow-xl shadow-slate-200/50 dark:shadow-none p-6 sm:p-8 space-y-6">
        ...
        <form method="POST" action="{{ route('login') }}" class="space-y-4">
            ...
            <!-- Mot de Passe avec Toggle Oeil Vanilla JS 100% Fiable -->
            <div class="space-y-1.5">
                ...
                <div class="relative">
                    <input id="password" 
                           ...
                           placeholder="••••••••••••"
                           class="w-full px-3.5 pr-11 py-2.5 rounded-xl bg-slate-50 dark:bg-[#0c101a] border @error('password') border-red-500 text-red-600 dark:text-red-400 @else border-slate-200 dark:border-slate-700/80 text-slate-900 dark:text-slate-100 @enderror text-xs font-mono placeholder-slate-400 focus:outline-none focus:border-[#0020B2] focus:ring-2 focus:ring-[#0020B2]/20 transition">
                    
                    <button type="button" 
                            id="togglePasswordBtn"
                            onclick="togglePasswordInput('password', this)"
                            aria-label="{{ __('Afficher ou masquer le mot de passe') }}"
                            title="{{ __('Afficher / Masquer le mot de passe') }}"
                        ...
                        <!-- Œil Ouvert (Afficher) -->
                        <svg id="eyeOpenIcon" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                            <path stroke-linecap="round" stroke-linejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                        </svg>
                        <!-- Œil Barré (Masquer) -->
                        <svg id="eyeClosedIcon" class="w-4 h-4 hidden" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
    ...
    <!-- Script Vanilla JS de Bascule du Mot de Passe (100% Garanti Sans Dépendance) -->
    <script>
        function togglePasswordInput(inputId, btn) {
            ...
            const eyeOpen = btn.querySelector('#eyeOpenIcon') || btn.querySelector('.eye-open');
            const eyeClosed = btn.querySelector('#eyeClosedIcon') || btn.querySelector('.eye-closed');
